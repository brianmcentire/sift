...
class TestListFiles:
    def test_returns_all_files_without_filters(self, client):
        insert_files(
            [
                make_file(
                    host="mac", path="/users/brian/a.txt", filename="a.txt", hash=HASH_A
                ),
    ...
    def test_path_prefix_no_false_match_on_similar_prefix(self, client):
        ...
        insert_files(
            [
                make_file(
                    host="mac", path="/users/brian/file.txt", filename="file.txt"
                ),
                make_file(
                    host="mac",
                    path="/users/brian2/file.txt",
                    filename="file.txt",
                    hash=HASH_B,
                ),
            ]
    ...
    def test_iname_glob_wildcard(self, client):
        insert_files(
            [
                ...
                make_file(
                    path="/a/report_2025.pdf", filename="report_2025.pdf", hash=HASH_B
                ),
    ...
    def test_other_hosts_populated_for_cross_host_dup(self, client):
        ...
        insert_files(
            [
                ...
                make_file(
                    host="nas", path="/mnt/photo.jpg", filename="photo.jpg", hash=HASH_C
                ),
            ]
        )
        resp = client.get("/files", params={"host": "mac"})
    ...
    def test_other_hosts_null_for_unique_file(self, client):
        insert_files(
            [make_file(host="mac", path="/users/brian/unique.txt", hash=HASH_B)]
        )
        resp = client.get("/files", params={"host": "mac"})
        assert resp.json()[0]["other_hosts"] is None

    def test_lite_mode_skips_other_hosts_enrichment(self, client):
        insert_files(
            [
                make_file(
                    host="mac",
        ...
        resp = client.get("/files", params={"host": "mac", "lite": True})
        assert resp.status_code == 200
        results = resp.json()
        assert len(results) == 1
        assert results[0]["other_hosts"] is None
    ...
    def test_min_size_filter(self, client):
        insert_files(
            [
                ...
                make_file(
                    path="/a/large.txt", filename="large.txt", size=10000, hash=HASH_B
                ),
            ]
        )
